...
func (c *Client) Enroll(req *api.EnrollmentRequest) (*EnrollmentResponse, error) {
	//nodelog.Logs("[Enroll] Fabric-CA-Client.Enroll In")
...
func (c *Client) handleX509Enroll(req *api.EnrollmentRequest) (*EnrollmentResponse, error) {
	// Generate the CSR
	//nodelog.Logs("[Enroll] Fabric-CA-Client.handleX509Enroll In")
	...
	//nodelog.Logs("[Enroll] Fabric-CA-Client.handleX509Enroll csrPEM:",string(csrPEM))
	...
	//nodelog.Logs("[Enroll] Fabric-CA-Client.handleX509Enroll newPost:",string(body))
	...
	//var CertKey string
	//CertKey = "enrollcert_"+req.Name
	//fmt.Println("获取缓存的 enrollcert Key :",CertKey)
	//
	//v,ok :=cache.GetStringValue(CertKey)
	//
	//if ok{
	//
	//	err = c.Init()
	...
	//
	//	if err == nil {
	//		fmt.Println("common.EnrollmentResponseNet", string(js))
	//		cache.SetStringValue(CertKey, string(js))
	//	}
	//}
	//fmt.Println("key:",key)
	//fmt.Println("cert:",result.Cert)
...
func (c *Client) newEnrollmentResponse(result *common.EnrollmentResponseNet, id string, key core.Key) (*EnrollmentResponse, error) {
	...
	//nodelog.Logs("[Enroll] Fabric-CA-Client.newEnrollmentResponse In")
...
func (c *Client) SendReq(req *http.Request, result interface{}) (err error) {
	//nodelog.Logs("[Enroll] Fabric-CA-Client.SendReq In")
	...
	if respBody != nil && len(respBody) > 0 {
		...
		//nodelog.Logs("[Enroll] Fabric-CA-Client.SendReq respBody ：",string(respBody))
