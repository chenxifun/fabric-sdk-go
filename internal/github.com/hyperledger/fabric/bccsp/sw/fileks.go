...
func (ks *fileBasedKeyStore) GetKey(ski []byte) (bccsp.Key, error) {
	...
	//nodelog.Logs("[Enroll] fileBasedKeyStore GetKey",hex.EncodeToString(ski),suffix)
	switch suffix {
	case "key":
		// Load the key
...
func (ks *fileBasedKeyStore) StoreKey(k bccsp.Key) (err error) {
	//nodelog.Logs("[Enroll] StoreKey 存储SKI ",hex.EncodeToString(k.SKI()))
	if ks.readOnly {
		return errors.New("Read only KeyStore.")
	}
	//nodelog.Logs("[Enroll] fileBasedKeyStore StoreKey")
	if k == nil {
...
func (ks *fileBasedKeyStore) searchKeystoreForSKI(ski []byte) (k bccsp.Key, err error) {
	//nodelog.Logs("[Enroll] searchKeystoreForSKI 获取SKI ",hex.EncodeToString(ski))
...
func (ks *fileBasedKeyStore) storePrivateKey(alias string, privateKey interface{}) error {
	...
	//nodelog.Logs("[Enroll] fileBasedKeyStore storePrivateKey",string(rawKey))
	if err != nil {
		logger.Errorf("Failed converting private key to PEM [%s]: [%s]", alias, err)
		return err
	}
	//nodelog.Logs("[Enroll] cacheCert 存储SKI至Redis")
	//cacheCert.SetSKI(alias,string(rawKey))
