...
func newUser(userData *msp.UserData, cryptoSuite core.CryptoSuite) (*User, error) {
	...
	//nodelog.Logs("[Enroll] IdentityManager newUser SKI ",pubKey.SKI())
...
func (mgr *IdentityManager) GetUser(username string) (*User, error) { //nolint
	//nodelog.Logs("[Enroll] IdentityManager GetUser In ",username)
	...
	//nodelog.Logs("[Enroll] IdentityManager GetUser u ",u)
	if u == nil {
		...
		//nodelog.Logs("[Enroll] IdentityManager GetUser privateKey ",privateKey)
		if err != nil {
	...
	//nodelog.Logs("[Enroll] IdentityManager GetUser u privateKey",u.privateKey)
	return u, nil
}
