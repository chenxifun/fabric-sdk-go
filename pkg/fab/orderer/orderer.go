...
func (o *Orderer) SendBroadcast(ctx reqContext.Context, envelope *fab.SignedEnvelope) (*common.Status, error) {
	//GatewayLog.Logs( "Orderer.SendBroadcast Start")
	conn, err := o.conn(ctx)
	if err != nil {
	...
	fmt.Println("gRPC 服务 Orderer.SendBroadcast 调用")
	fmt.Println("conn.Target ", conn.Target())
	broadcastClient, err := ab.NewAtomicBroadcastClient(conn).Broadcast(ctx) //,grpc.UseCompressor("gzip")
	if err != nil {
		rpcStatus, ok := grpcstatus.FromError(err)
		if ok {
	...
	//GatewayLog.Logs("Orderer.SendBroadcast End")
...
read:
	...
	//GatewayLog.Logs("Orderer.wrapStreamStatusRPC End")
...
func (o *Orderer) SendDeliver(ctx reqContext.Context, envelope *fab.SignedEnvelope) (chan *common.Block, chan error) {
	...
	fmt.Println("gRPC 服务 Orderer.SendDeliver 调用")
	fmt.Println("conn.Target ", conn.Target())
	// Create atomic broadcast client
	broadcastClient, err := ab.NewAtomicBroadcastClient(conn).Deliver(ctx) //,grpc.UseCompressor("gzip")
	if err != nil {
