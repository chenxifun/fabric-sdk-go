...
func (c *Client) Send(ctx context.Context, req *discclient.Request, targets ...fab.PeerConfig) ([]Response, error) {
	//GatewayLog.Logs("discoveryClient Send ")
	if len(targets) == 0 {
	...
	var errs error
	//GatewayLog.Logs( "discoveryClient Send targets Len ",len(targets))
	for _, t := range targets {

		//GatewayLog.Logs( "discoveryClient  ",t.URL)
	...
	wg.Wait()
	//GatewayLog.Logs( "discoveryClient End ")
...
func (c *Client) send(reqCtx context.Context, req *discclient.Request, target fab.PeerConfig) (discclient.Response, error) {
	...
	//GatewayLog.Logs( "discoveryClient send  End")
